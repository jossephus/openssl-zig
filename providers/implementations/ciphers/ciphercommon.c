...
int ossl_cipher_aead_get_ctx_params_find_pidx(const char *s) {
    switch(s[0]) {
    default: break;
    case 'i': if (strcmp("vlen", s+1) == 0) return 0; break;
    case 'k': if (strcmp("eylen", s+1) == 0) return 1; break;
    case 't': if (s[1] == 'a' && s[2] == 'g') {
        if (strcmp("len", s+3) == 0) return 2;
        if (s[3] == '\0') return 5;
    } break;
    }
    return -1;
...
int ossl_cipher_aead_set_ctx_params_find_pidx(const char *s) {
    ...
    default: break;
    case 'i': if (strcmp("vlen", s+1) == 0) return 0; break;
    case 't': if (s[1] == 'a' && s[2] == 'g') return 1; break;
    }
    return -1;
}
